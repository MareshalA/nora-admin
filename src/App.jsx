...
function ConversationLive({ phone, name, tenantId, tenant, token, onBack, onTenantRefresh }) {
  ...
  const sendManual = async () => {
    ...
    try {
      ...
      if (pending && pending.length > 0) {
        await SB.patch("conversations", `id=eq.${pending[0].id}`, { bot_response: replyText, bot_active: "Pas actif", reply_channel: "saas" }, token);
      } else {
        await SB.post("conversations", { tenant_id: tenantId, customer_phone: phone, customer_name: name, client_message: null, bot_response: replyText, bot_active: "Pas actif", reply_channel: "saas", booking_link_sent: "non" }, token);
      }
  ...
  const markWhatsApp = async () => {
    ...
    if (pending && pending.length > 0) {
      await SB.patch("conversations", `id=eq.${pending[0].id}`, { bot_active: "Pas actif", reply_channel: "whatsapp" }, token);
    }
  };

  const isSalon = (m) => m.bot_active === "Pas actif";
          ...
          <div key={m.id}>
            ...
            {m.bot_response && (
              <div style={{ display: "flex", justifyContent: "flex-end" }}>
                <div style={{ maxWidth: "78%" }}>
                  <div style={{ background: isSalon(m) ? "var(--amber-bg)" : "var(--accent-bg)", border: `1px solid ${isSalon(m) ? "var(--amber)" : "var(--accent)"}40`, borderRadius: "12px 12px 4px 12px", padding: "8px 12px" }}>
                    <div style={{ fontSize: 13 }}>{m.bot_response}</div>
                  </div>
                  <div style={{ fontSize: 10, color: "var(--t3)", marginTop: 2, paddingRight: 4, textAlign: "right" }}>
                    {isSalon(m) ? `🧑‍💼 Salon${m.reply_channel === "whatsapp" ? " (WhatsApp)" : ""}` : "🤖 Bot"} · {fmtFull(m.timestamp)}
                  </div>
                </div>
              </div>
            )}
...
function LiveTab({ conversations, tenantId, tenant, token, onTenantRefresh }) {
  ...
  const pausedActive = paused.filter(phone => byPhone[phone]);

  const handleResume = async (e, phone) => {
    e.stopPropagation();
    setResuming(phone);
    const newPaused = paused.filter(p => p !== phone);
    await SB.patch("tenants", `id=eq.${tenantId}`, { paused_phones: newPaused }, token);
    await onTenantRefresh();
    setResuming(null);
  };
  ...
  return (
    <div className="fade-in">
      ...
      {/* Bannière pauses actives */}
      {pausedActive.length > 0 && (
        <div style={{ background: "var(--amber-bg)", border: "1px solid rgba(245,166,35,0.4)", borderRadius: 12, padding: "12px 14px", marginBottom: 14 }}>
          <div style={{ fontSize: 12, fontWeight: 600, color: "var(--amber)", marginBottom: 8 }}>
            ⏸ {pausedActive.length} bot{pausedActive.length > 1 ? "s" : ""} en pause
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            {pausedActive.map(phone => {
              const c = byPhone[phone];
              return (
                <div key={phone} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", background: "rgba(245,166,35,0.08)", borderRadius: 8, padding: "8px 10px" }}>
                  <div>
                    <div style={{ fontSize: 13, fontWeight: 600, color: "var(--t1)" }}>{c?.customer_name || phone}</div>
                    <div style={{ fontSize: 11, color: "var(--t3)" }}>{phone}</div>
                  </div>
                  <button
                    onClick={e => handleResume(e, phone)}
                    disabled={resuming === phone}
                    style={{ ...btnPrimary, padding: "5px 12px", fontSize: 12, opacity: resuming === phone ? 0.5 : 1 }}
                  >
                    {resuming === phone ? "..." : "Reprendre bot"}
                  </button>
                </div>
              );
            })}
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {clientMsgs.map(m => (
            <div key={m.id} style={{ background: "var(--surface)", border: "1px solid var(--bd)", borderRadius: 12, padding: 12 }}>
              ...
              {m.bot_response && <div style={{ fontSize: 13 }}>{m.bot_active === "Pas actif" ? "🧑‍💼" : "🤖"} {m.bot_response}</div>}
...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    const iv = setInterval(async () => {
      const convs = await SB.get("conversations", `tenant_id=eq.${tenantId}&select=*&order=timestamp.desc&limit=100`, token);
        ...
        <div className="mobile-nav" style={{ display: "none", position: "fixed", bottom: 0, left: 0, right: 0, background: "var(--surface)", borderTop: "1px solid var(--bd)", justifyContent: "space-around", alignItems: "center", padding: "4px 0 env(safe-area-inset-bottom, 4px)", zIndex: 100 }}>
          {tabs.map(t => (
            <button key={t.id} onClick={() => setTab(t.id)} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 1, border: "none", cursor: "pointer", padding: "6px 4px", borderRadius: 8, background: "transparent", color: tab === t.id ? "var(--accent2)" : "var(--t3)", fontSize: 9, fontWeight: tab === t.id ? 600 : 400, position: "relative" }}>
              ...
              {t.label}
              {t.badge && <span style={{ position: "absolute", top: 2, right: 2, fontSize: 9, background: t.badgePause ? "var(--amber)" : "var(--accent)", color: "#fff", borderRadius: 8, padding: "0 4px" }}>{t.badge}</span>}
            </button>
          ))}
        </div>
      </div>
