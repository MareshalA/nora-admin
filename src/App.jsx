...
const tag = (label, color, bg) => ({ fontSize: 10, padding: "2px 7px", borderRadius: 6, background: bg, color, fontWeight: 500 });
  ...
  return (
    <div className="fade-in">
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
        ...
        <button onClick={startNew} style={{ ...btnPrimary, padding: "9px 14px", fontSize: 13 }}>+ Ajouter</button>
      </div>
      {editing && (
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 8 }}>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
                  ...
                  {o.categorie && <span style={tag(o.categorie, "var(--accent2)", "var(--accent-bg)")}>{o.categorie}</span>}
                  {!o.active && <span style={tag("Off", "var(--red)", "var(--red-bg)")}>Off</span>}
                </div>
...
// ─── Appointments (Liste + Agenda) ───
function AppointmentsTab({ appointments, token, onRefresh }) {
  const [filter, setFilter] = useState("all");
  const [view, setView] = useState("list");
  const filtered = filter === "all" ? appointments : appointments.filter(a => a.status === filter);
  const sc = { confirmé: "var(--accent)", cancelled: "var(--red)", completed: "var(--blue)", no_show: "var(--amber)", confirmed: "var(--accent)" };
  ...
  // Agenda — groupe les RDV par jour
            ...
            return (
              <div key={day.toISOString()}>
                ...
                {appts.length === 0 ? (
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                    {appts.map(a => (
                      <div key={a.id} style={{ background: "var(--surface)", border: `1px solid ${sc[a.status] || "var(--bd)"}40`, borderLeft: `3px solid ${sc[a.status] || "var(--bd)"}`, borderRadius: "0 10px 10px 0", padding: "10px 14px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                        <div>
...
function ConversationLive({ phone, name, tenantId, tenant, token, onBack }) {
  ...
  const checkPaused = useCallback(() => {
    ...
    setBotPaused(paused.includes(phone));
  }, [phone, tenant]);

  const loadMessages = useCallback(async () => {
    const data = await SB.get("conversations", `tenant_id=eq.${tenantId}&customer_phone=eq.${encodeURIComponent(phone)}&order=timestamp.asc&limit=100`, token);
    if (Array.isArray(data)) setMessages(data);
    checkPaused();
  }, [phone, tenantId, token, checkPaused]);
  ...
  const sendManual = async () => {
    ...
    setSending(true);
    try {
      const pending = await SB.get("conversations", `tenant_id=eq.${tenantId}&customer_phone=eq.${encodeURIComponent(phone)}&bot_response=is.null&order=timestamp.desc&limit=1`, token);
      if (pending && pending.length > 0) {
        await SB.patch("conversations", `id=eq.${pending[0].id}`, { bot_response: replyText, bot_active: false, reply_channel: "saas" }, token);
      } else {
        await SB.post("conversations", { tenant_id: tenantId, customer_phone: phone, customer_name: name, client_message: null, bot_response: replyText, bot_active: false, reply_channel: "saas", booking_link_sent: "non" }, token);
      }
      await fetch(N8N_SEND_WHATSAPP, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ to: phone, from: tenant?.phone_whatsapp, message: replyText }) });
      setReplyText("");
      await loadMessages();
  ...
  const markWhatsApp = async () => {
    const pending = await SB.get("conversations", `tenant_id=eq.${tenantId}&customer_phone=eq.${encodeURIComponent(phone)}&bot_response=is.null&order=timestamp.desc&limit=1`, token);
    if (pending && pending.length > 0) {
      await SB.patch("conversations", `id=eq.${pending[0].id}`, { bot_active: false, reply_channel: "whatsapp" }, token);
    }
  };
    ...
    <div className="fade-in" style={{ display: "flex", flexDirection: "column", height: "calc(100vh - 96px)" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 10, paddingBottom: 12, borderBottom: "1px solid var(--bd)", marginBottom: 12, flexShrink: 0 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
          ...
          {!botPaused
            ? <button onClick={pauseBot} style={{ ...btnSecondary, padding: "6px 10px", fontSize: 11, color: "var(--amber)", borderColor: "var(--amber)" }}>Pause bot</button>
            : <button onClick={resumeBot} style={{ ...btnPrimary, padding: "6px 10px", fontSize: 11 }}>Reprendre bot</button>
          }
        </div>
      </div>
            ...
            {m.client_message && (
              <div style={{ display: "flex", justifyContent: "flex-start", marginBottom: 4 }}>
                <div style={{ maxWidth: "78%" }}>
                  ...
                  <div style={{ fontSize: 10, color: "var(--t3)", marginTop: 2, paddingLeft: 4 }}>{fmtFull(m.timestamp)}</div>
                </div>
              </div>
            )}
            {m.bot_response && (
        ...
        <div style={{ borderTop: "1px solid var(--bd)", paddingTop: 12, marginTop: 8, flexShrink: 0 }}>
          ...
          {replyMode === "saas" ? (
            <div style={{ display: "flex", gap: 8 }}>
              ...
              <button onClick={sendManual} disabled={sending || !replyText.trim()} style={{ ...btnPrimary, padding: "10px 14px", fontSize: 13, opacity: sending || !replyText.trim() ? 0.5 : 1, alignSelf: "flex-end" }}>
                {sending ? "..." : "Envoyer"}
              </button>
            </div>
          ) : (
            <div style={{ background: "var(--surface2)", border: "1px solid var(--amber)40", borderRadius: 10, padding: 12 }}>
...
// ─── Live Tab ───
function LiveTab({ conversations, tenantId, tenant, token }) {
  const [selected, setSelected] = useState(null);
  const paused = tenant?.paused_phones || [];
  const now = new Date();

  // Une ligne par client — dernier message < 30 min
  const byPhone = {};
  ...
  const live = Object.values(byPhone).filter(c => {
    const diff = (now - new Date(c.timestamp)) / 60000;
    return diff <= 30;
  ...
  return (
    <div className="fade-in">
      ...
      {pausedActive.length > 0 && (
        <div style={{ background: "var(--amber-bg)", border: "1px solid var(--amber)40", borderRadius: 12, padding: "10px 14px", marginBottom: 12, display: "flex", alignItems: "center", gap: 8 }}>
      ...
      {live.length === 0 ? (
        <div style={{ textAlign: "center", padding: 48, color: "var(--t3)" }}>
          ...
          <div style={{ fontSize: 12, marginTop: 4 }}>Les conversations actives des 30 dernières minutes apparaîtront ici</div>
        </div>
      ) : (
        <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
          {live.map(c => {
              ...
              <div key={c.customer_phone} onClick={() => setSelected({ phone: c.customer_phone, name: c.customer_name })} style={{ background: "var(--surface)", border: `1px solid ${isPaused ? "var(--amber)" : "var(--bd)"}`, borderRadius: 12, padding: 14, cursor: "pointer" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 6 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                    ...
                    {isPaused && <span style={tag("⏸ Pause", "var(--amber)", "var(--amber-bg)")}>⏸ Pause</span>}
                  </div>
                  <span style={{ fontSize: 11, color: "var(--t3)" }}>il y a {diff} min</span>
                </div>
                <div style={{ fontSize: 12, color: "var(--t2)", marginBottom: 2 }}>{(c.client_message || "").slice(0, 80)}</div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {clientMsgs.map(m => (
            <div key={m.id} style={{ background: "var(--surface)", border: "1px solid var(--bd)", borderRadius: 12, padding: 12 }}>
              ...
              {m.bot_response && <div style={{ fontSize: 13, color: "var(--t1)" }}>{m.bot_active === false ? "🧑‍💼" : "🤖"} {m.bot_response}</div>}
              <div style={{ display: "flex", gap: 4, marginTop: 6, flexWrap: "wrap" }}>
                {m.booking_link_sent === "oui" && <span style={tag("Lien", "var(--accent2)", "var(--accent-bg)")}>Lien</span>}
                {m.interest && <span style={tag(m.interest, "var(--blue)", "var(--blue-bg)")}>{m.interest}</span>}
              </div>
            </div>
          ))}
        </div>
        ...
        {unique.slice(0, 100).map(c => (
          <div key={c.customer_phone} onClick={() => setSelected({ phone: c.customer_phone, name: c.customer_name })} style={{ background: "var(--surface)", border: "1px solid var(--bd)", borderRadius: 12, padding: 12, cursor: "pointer" }}>
            ...
            <div style={{ display: "flex", gap: 4, marginTop: 5, flexWrap: "wrap" }}>
              {c.booking_link_sent === "oui" && <span style={tag("Lien", "var(--accent2)", "var(--accent-bg)")}>Lien</span>}
              {c.interest && <span style={tag(c.interest, "var(--blue)", "var(--blue-bg)")}>{c.interest}</span>}
            </div>
          </div>
        ))}
...
export default function AdminDashboard() {
  const [auth, setAuth] = useState(null); const [tenant, setTenant] = useState(null); const [tab, setTab] = useState("dashboard"); const [loading, setLoading] = useState(false);
  const [offers, setOffers] = useState([]); const [availability, setAvailability] = useState([]); const [appointments, setAppointments] = useState([]); const [conversations, setConversations] = useState([]); const [fidelite, setFidelite] = useState([]);
  ...
  const token = auth?.access_token; const tenantId = tenant?.id;
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [off, avail, appts, convs, fid, tenantFresh] = await Promise.all([
        ...
        SB.get("conversations", `tenant_id=eq.${tenantId}&select=*&order=timestamp.desc&limit=500`, token),
  ...
  // Rafraîchir tenant toutes les 30s pour paused_phones à jour
  useEffect(() => {
    if (!tenantId || !token) return;
  ...
  const now = new Date();
  const byPhone = {};
  [...conversations].forEach(c => { if (!byPhone[c.customer_phone] || new Date(c.timestamp) > new Date(byPhone[c.customer_phone].timestamp)) byPhone[c.customer_phone] = c; });
  const liveCount = Object.values(byPhone).filter(c => (now - new Date(c.timestamp)) / 60000 <= 30).length;
      ...
      <div style={{ display: "flex", minHeight: "100vh" }}>
        <div className="desktop-sidebar" style={{ width: 190, background: "var(--surface)", borderRight: "1px solid var(--bd)", padding: "20px 10px", flexDirection: "column", flexShrink: 0 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 2, flex: 1 }}>
            {tabs.map(t => (
              <button key={t.id} onClick={() => setTab(t.id)} style={{ display: "flex", alignItems: "center", gap: 8, padding: "9px 10px", borderRadius: 8, border: "none", cursor: "pointer", fontSize: 13, fontWeight: 500, background: tab === t.id ? "var(--accent-bg)" : "transparent", color: tab === t.id ? "var(--accent2)" : "var(--t2)", textAlign: "left", width: "100%", position: "relative" }}>
        ...
        <div className="main-content" style={{ flex: 1, padding: 24, overflowY: "auto", maxHeight: "100vh" }}>
          ...
          {tab === "live" && <LiveTab conversations={conversations} tenantId={tenantId} tenant={tenant} token={token} />}
        ...
        <div className="mobile-nav" style={{ display: "none", position: "fixed", bottom: 0, left: 0, right: 0, background: "var(--surface)", borderTop: "1px solid var(--bd)", justifyContent: "space-around", alignItems: "center", padding: "4px 0 env(safe-area-inset-bottom, 4px)", zIndex: 100 }}>
          {tabs.map(t => (
            <button key={t.id} onClick={() => setTab(t.id)} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 1, border: "none", cursor: "pointer", padding: "6px 4px", borderRadius: 8, background: "transparent", color: tab === t.id ? "var(--accent2)" : "var(--t3)", fontSize: 9, fontWeight: tab === t.id ? 600 : 400, position: "relative" }}>
              ...
              {t.label}
              {t.badge && <span style={{ position: "absolute", top: 2, right: 2, fontSize: 9, background: "var(--accent)", color: "#fff", borderRadius: 8, padding: "0 4px", fontWeight: 700 }}>{t.badge}</span>}
            </button>
          ))}
        </div>
      </div>
