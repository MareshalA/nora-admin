...
// ─── Plans — chargés depuis Supabase au démarrage ────────────────────────────
// hasFeat utilise les colonnes feature_* du tenant directement
const hasFeat = (tenant, f) => {
  if (!tenant) return false;
  if (f === "live")     return !!tenant.feature_live_enabled;
  if (f === "relances") return !!tenant.feature_reminders_enabled;
  if (f === "avis")     return !!tenant.feature_reviews_enabled;
  if (f === "upsell")   return !!tenant.feature_upsell_enabled;
  return true;
};

const PLAN_COLORS = {
  trial: "#7A8478", starter: "#5C6960", pro: "#1D9E75",
  business: "#E89B5A", sur_mesure: "#2A6FDB"
};
...
@media(max-width:768px){
  ...
  .mn{padding:14px 12px 80px}
  .kg{grid-template-columns:1fr 1fr}
  .g2{grid-template-columns:1fr!important}
  .hide-mob{display:none!important}
  .tbl-wrap{overflow-x:auto;-webkit-overflow-scrolling:touch}
  .mob-card{display:block!important}
  .desk-tbl{display:none!important}
}
@media(min-width:769px){
  .mob-card{display:none!important}
}
...
function Dashboard({ stats, conversations, appointments, tenant }) {
  ...
  const convLim = tenant?.conv_monthly_limit ?? 300;
  const cpct  = convLim ? Math.min(Math.round((convM / convLim) * 100), 100) : 0;
    ...
    <div className="fade">
      ...
      <div className="card">
        <div className="ch">
          ...
          <span className="pill" style={{ background: `${PLAN_COLORS[plan] || "#5C6960"}15`, color: PLAN_COLORS[plan] || "#5C6960", fontFamily: "var(--mn)" }}>{plan}</span>
        </div>
        <div style={{ padding: "13px 16px", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }} className="g2">
          <div>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, fontFamily: "var(--mn)", color: "var(--i3)", marginBottom: 5 }}>
              <span>{convM} utilisées</span><span style={{ color: cc }}>{convLim ? `/ ${convLim}` : "Illimité"}</span>
            </div>
            {convLim && <>
    ...
    <div className="fade">
      <div className="sh">
        ...
        <div style={{ display: "flex", gap: 7, flexWrap: "wrap" }}>
          ...
          <button className="btn bg2 sm" onClick={() => exportCSV(fil.map(a => ({ Client: a.customer_name || "", Tel: a.customer_phone || "", Service: a.service_name || "", Date: fmtD(a.scheduled_at, { day: "2-digit", month: "2-digit", year: "numeric" }), Heure: fmtH(a.scheduled_at), Statut: STATUS_LABEL[a.status] || a.status || "", Praticien: a.personnel_nom || "" })), "rdv-nora.csv")}>Exporter CSV</button>
        </div>
      </div>
      ...
      {view === "list" && (
        <>
          ...
          <div className="card desk-tbl" style={{ overflow: "hidden" }}>
            {fil.length === 0
                  ...
                  <tbody>
                    {fil.map(a => { const p = getP(a.personnel_id); return (
                      <tr key={a.id}>
                        ...
                        <td>
                          {p
                            ? <div style={{ display: "flex", alignItems: "center", gap: 6 }}><Av prenom={p.prenom} nom={p.nom} size={24} couleur={p.couleur} /><span style={{ fontSize: 12 }}>{p.prenom}</span></div>
                            : personnel.length > 1
                              ? <select className="inp ism" style={{ fontSize: 12, padding: "4px 8px" }} value="" onChange={async e => { if (!e.target.value) return; const psel = personnel.find(x => x.id === e.target.value); await SB.patch("appointments", `id=eq.${a.id}`, { personnel_id: e.target.value, personnel_nom: psel ? `${psel.prenom} ${psel.nom || ""}`.trim() : "" }, token); onRefresh(); }}>
                                  <option value="">Affecter…</option>
                                  {personnel.map(px => <option key={px.id} value={px.id}>{px.prenom} {px.nom || ""}</option>)}
                                </select>
                              : <span style={{ fontSize: 12, color: "var(--i3)" }}>{a.personnel_nom || "—"}</span>
                          }
                        </td>
          ...
          {/* Mobile cards */}
          <div className="mob-card" style={{ display: "none" }}>
            {fil.map(a => {
              const p = getP(a.personnel_id);
              return (
                <div key={a.id} style={{ background: "var(--s1)", border: "1px solid var(--ln)", borderRadius: 10, padding: "12px 14px", marginBottom: 8 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 8 }}>
                    <div>
                      <div style={{ fontWeight: 700, fontSize: 14 }}>{a.customer_name || "—"}</div>
                      <div style={{ fontSize: 11, color: "var(--i3)", fontFamily: "var(--mn)" }}>{a.customer_phone}</div>
                    </div>
                    <SPill status={a.status} />
                  </div>
                  <div style={{ fontSize: 13, marginBottom: 4 }}>{a.service_name}</div>
                  <div style={{ fontFamily: "var(--mn)", fontSize: 12, color: "var(--br)", fontWeight: 600, marginBottom: 8 }}>
                    {fmtD(a.scheduled_at, { day: "2-digit", month: "short" })} à {fmtH(a.scheduled_at)}
                  </div>
                  {isConfirmed(a.status) && (
                    <div style={{ display: "flex", gap: 6 }}>
                      <button className="btn bg2 sm" onClick={() => setConf({ id: a.id, action: "terminé", lbl: `Terminer le RDV de ${a.customer_name || "ce client"} ?` })}>Terminé</button>
                      <button className="btn bg2 sm" style={{ color: "var(--am)" }} onClick={() => setConf({ id: a.id, action: "absent", lbl: "Marquer absent ?" })}>Absent</button>
                      <button className="btn bg2 sm" style={{ color: "var(--rd)" }} onClick={() => setConf({ id: a.id, action: "annulé", lbl: "Annuler ?" })}>Annuler</button>
                    </div>
                  )}
                  {personnel.length > 1 && !a.personnel_id && (
                    <select className="inp ism" style={{ marginTop: 8, fontSize: 12 }} value="" onChange={async e => { if (!e.target.value) return; const psel = personnel.find(x => x.id === e.target.value); await SB.patch("appointments", `id=eq.${a.id}`, { personnel_id: e.target.value, personnel_nom: psel ? `${psel.prenom} ${psel.nom || ""}`.trim() : "" }, token); onRefresh(); }}>
                      <option value="">Affecter un praticien…</option>
                      {personnel.map(px => <option key={px.id} value={px.id}>{px.prenom} {px.nom || ""}</option>)}
                    </select>
                  )}
                </div>
...
// ─── PERSONNEL ────────────────────────────────────────────────────────────────
function PersonnelTab({ personnel, offers, tenantId, tenant, token, onRefresh }) {
  const [ed, setEd] = useState(null); const [fm, setFm] = useState({});
  const praticienLimit = tenant?.praticien_limit ?? 1;
  const actifs = personnel.filter(p => p.statut !== "inactif").length;
  const canAdd = !praticienLimit || actifs < praticienLimit;
  ...
  return (
    <div className="fade">
      <div className="sh">
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          {praticienLimit && <span style={{ fontFamily: "var(--mn)", fontSize: 11, color: "var(--i3)" }}>{actifs} / {praticienLimit} praticiens</span>}
          {canAdd
...
function HistoriqueTab({ conversations, appointments }) {
  ...
  return (
    <div className="fade">
      <div className="sh"><div><div className="ey">Clients</div><div className="st">Historique</div></div><button className="btn bg2 sm" onClick={csv}>Exporter CSV</button></div>
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 10 }}>
        <input className="inp ism" style={{ maxWidth: 190 }} placeholder="Nom ou numéro…" value={search} onChange={e => setSearch(e.target.value)} />
        <input type="date" className="inp ism" style={{ maxWidth: 136 }} value={df} onChange={e => setDf(e.target.value)} />
        <span style={{ alignSelf: "center", color: "var(--i3)", fontSize: 12 }}>→</span>
...
// ─── ABONNEMENT ───────────────────────────────────────────────────────────────
function AbonnementTab({ tenant, conversations, payments }) {
  const plan      = tenant?.plan || "trial";
  const planColor = PLAN_COLORS[plan] || "#5C6960";
  const convLimit = tenant?.conv_monthly_limit ?? 300;
  const trL       = daysLeft(tenant?.trial_ends_at);
  ...
  const cpct      = convLimit ? Math.min(Math.round((convM / convLimit) * 100), 100) : 0;
  const cc        = cpct > 90 ? "var(--rd)" : cpct > 70 ? "var(--am)" : "var(--br)";
  const WA_UPGRADE = "https://wa.me/212675235528?text=Salam%2C%20je%20voudrais%20upgrader%20mon%20abonnement%20Nora";
  const [period, setPeriod] = useState("m");
  const mult = period === "m" ? 1 : period === "6" ? .85 : .75;

  const ALL_PLANS = [
    { key: "starter",    label: "Starter",    angle: "Je démarre",              base: 399,  convLimit: 300,  praticiens: "1 praticien",
      feats: ["Bot WhatsApp FR · AR · Darija", "Réservation automatique 24h/24", "Confirmation & rappels par mail", "Dashboard & KPIs de base"],
      locked: ["Avis Google post-RDV", "Upsell post-RDV", "Live + pause bot", "Analytics avancés"] },
    { key: "pro",        label: "Pro",         angle: "Je veux plus de clients", base: 599,  convLimit: 1000, praticiens: "5 praticiens",
      feats: ["Tout Starter", "Demande d'avis Google après RDV", "Upsell post-RDV", "Live + pause bot par client", "Analytics offres"],
      locked: ["Analytics prédictif", "Programme fidélité", "Bot personnalisé"] },
    { key: "business",   label: "Business",    angle: "Je pilote mon activité",  base: 999,  convLimit: null, praticiens: "Illimité",
      feats: ["Tout Pro", "Analytics prédictif", "Programme fidélité points", "Bot personnalisé", "Support prioritaire"],
      locked: [] },
    { key: "sur_mesure", label: "Sur mesure",  angle: "Groupe & franchise",      base: null, convLimit: null, praticiens: "Illimité",
      feats: ["Tout Business", "Volume négocié", "Account manager dédié", "Facturation personnalisée"],
      locked: [] },
  ];
  ...
  return (
    <div className="fade">
      ...
      {!inTrial && (
        <div className="card" style={{ marginBottom: 14, borderColor: planColor, borderWidth: 1.5 }}>
          <div style={{ padding: "15px 18px", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
            <div>
              ...
              <div style={{ fontSize: 24, fontWeight: 800, letterSpacing: "-.03em", color: planColor }}>{plan.charAt(0).toUpperCase() + plan.slice(1).replace("_", " ")}</div>
              <div style={{ fontSize: 12, color: "var(--i3)", fontFamily: "var(--mn)", marginTop: 2 }}>{tenant?.plan_expire_at ? `Renouvellement le ${fmtD(tenant.plan_expire_at, { day: "numeric", month: "long", year: "numeric" })}` : "Actif"}</div>
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
              <div>
                ...
                <div style={{ display: "flex", alignItems: "baseline", gap: 5, marginBottom: 6 }}>
                  ...
                  {convLimit && <span style={{ fontSize: 13, color: "var(--i3)" }}>/ {convLimit}</span>}
                </div>
                {convLimit ? (<><div className="ptr" style={{ marginBottom: 4 }}><div className="ptf" style={{ width: `${cpct}%`, background: cc }} /></div><div style={{ fontSize: 10, color: cc, fontFamily: "var(--mn)" }}>{cpct}%{cpct > 90 && " — limite proche !"}</div></>) : <div style={{ fontSize: 12, color: "var(--br)", fontFamily: "var(--mn)" }}>Illimité</div>}
              </div>
              <div>
          ...
          {cpct > 80 && <div style={{ margin: "0 18px 14px", padding: "9px 13px", background: "var(--ams)", border: "1px solid rgba(201,122,47,.3)", borderRadius: "var(--r1)", fontSize: 12, color: "var(--am)" }}>Vous approchez de la limite mensuelle. <a href={WA_UPGRADE} target="_blank" rel="noopener" style={{ color: "var(--am)", fontWeight: 600 }}>Contacter Nora pour upgrader →</a></div>}
        </div>
      )}
        ...
        {ALL_PLANS.map(p => {
          ...
          return (
            <div key={p.key} className={`pc ${isCur ? "cur" : ""}`}>
              ...
              <div style={{ fontSize: 18, fontWeight: 700, letterSpacing: "-.02em", marginBottom: 8, color: isCur ? (PLAN_COLORS[p.key] || "var(--br)") : "var(--ink)" }}>{p.label}</div>
              {price
              ...
              {isCur
                ...
                : p.key === "sur_mesure"
                  ? <a href={WA_UPGRADE} target="_blank" rel="noopener" className="btn bg2 sm" style={{ width: "100%", justifyContent: "center", textDecoration: "none" }}>Nous contacter →</a>
                  : isDown
                    ? <button className="btn bg2 sm" style={{ width: "100%", justifyContent: "center" }}>Rétrograder</button>
                    : <a href={WA_UPGRADE} target="_blank" rel="noopener" className="btn bp sm" style={{ width: "100%", justifyContent: "center", textDecoration: "none" }}>Passer à {p.label} →</a>}
            </div>
          );
        })}
      </div>
      ...
      <div className="card" style={{ overflow: "hidden" }}>
        <table className="tbl">
          <thead><tr><th>Date</th><th>Description</th><th>Mode</th><th style={{ textAlign: "right" }}>Montant</th></tr></thead>
          <tbody>
            {(payments || []).map((p) => (
              <tr key={p.id}>
                <td style={{ fontFamily: "var(--mn)", fontSize: 11 }}>{fmtD(p.payment_date, { day: "2-digit", month: "short", year: "numeric" })}</td>
                <td>{p.notes || `Plan ${plan} · Mensuel`}</td>
                <td><span className="pill" style={{ background: "var(--s2)", color: "var(--i3)", fontSize: 10 }}>{p.method}</span></td>
                <td style={{ fontFamily: "var(--mn)", fontWeight: 600, textAlign: "right", color: "var(--br)" }}>{(p.amount_mad || 0).toLocaleString("fr-FR")} MAD</td>
              </tr>
            ))}
            {!(payments || []).length && <tr><td colSpan={4} style={{ textAlign: "center", padding: 24, color: "var(--i3)", fontSize: 13 }}>Aucun paiement enregistré</td></tr>}
          </tbody>
        </table>
      </div>
    </div>
...
export default function App() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [off, cats, avail, appts, convs, pers, tFresh, pays] = await Promise.all([
        ...
        SB.get("tenant_payments", `tenant_id=eq.${tenantId}&select=*&order=payment_date.desc&limit=50`, token),
      ]);
      setOffers(arr(off)); setCategories(arr(cats)); setAvail(arr(avail));
      setAppts(arr(appts)); setConvs(arr(convs)); setPersonnel(arr(pers)); setPayments(arr(pays));
  ...
  const TABS = [
    ...
    ...(tenant?.praticien_limit > 1 ? [{ id: "personnel", icon: "◉", label: "Personnel" }] : []),
    ...
    <>
      ...
      <div className="ly">
        ...
        <main className="mn">
          ...
          {tab === "personnel"  && <PersonnelTab personnel={personnel} offers={offers} tenantId={tenantId} tenant={tenant} token={token} onRefresh={loadData} />}
          ...
          {tab === "abonnement" && <AbonnementTab tenant={tenant} conversations={conversations} payments={payments} />}
